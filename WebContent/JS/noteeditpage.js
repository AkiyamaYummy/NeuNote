...
function setMainEdit() {
    ...
    $(topbuttons[3]).click(function () {
        shareNoteWithBackend();
    });
    $(topbuttons[4]).click(function () {
        if(mainEdit.showing)
            notShareNoteWithBackend(mainEdit.showing.note);
    });
    bindNoteDisplayItems();
}
function bindNoteDisplayItems(mode) {
    $("#top-panel-note-prev-button").click(function () {
        if(mainEdit.showing.pageleft > 1){
            showingChange(mainEdit.showing.note,mainEdit.showing.pageleft-1,mainEdit.showing.pageleft,mode);
        }
    });
    $("#top-panel-note-next-button").click(function () {
        if(mainEdit.showing.pageleft < mainEdit.notemessage[mainEdit.showing.note].pages){
            showingChange(mainEdit.showing.note,mainEdit.showing.pageleft+1,mainEdit.showing.pageleft+2,mode);
        }
    });
    $("#showing-note-left-page").keydown(function (event) {
        if(event.which === 13) showingTextboxChange(mode);
    });
    $("#showing-note-right-page").keydown(function (event) {
        if(event.which === 13) showingTextboxChange(mode);
    });
}
function showingTextboxChange(mode) {
    ...
    if(/[0-9]+/.test(leftstr) && /[0-9]+/.test(rightstr)){
        ...
        if(leftval>0 && leftval <= totpage && rightval>0 && rightval<=totpage+1){
            showingChange(mainEdit.showing.note,leftval,rightval,mode);
            return;
        }
    }
...
function showingChange(id,pageleft,pageright,mode) {
    if(!mode || mode!=="readonly") saveNowEditingPageWithBackend();
    getBackendPage(id,pageleft,$("#page1"),mode);
    getBackendPage(id,pageright,$("#page2"),mode);
...
function getBackendNote(id,mode) {
    var c_code = get_cookie("c_code");
    if(c_code === null){
        location.href = "login.jsp";
    }
    var urlstr = "NoteEdit";
    if(mode && mode === "readonly")urlstr = "ShareNote";
    $.ajax({
        type: 'POST',
        dataType: 'text',
        scriptCharset: 'utf-8',
        ...
        success:function(data){
            if((data+"").trim() === "You are not logged in."){
                ...
                if(!mode || mode!=="readonly")showTreeEditPanel();
            }
        }.bind(this)
    });
}
    ...
    $.ajax({
        ...
        success:function(data){
            if((data+"").trim() === "You are not logged in."){
                ...
                var notemessage = JSON.parse((data+"").trim());
                mainEdit.notemessage[notemessage.noteId] = notemessage;
                if(mainEdit.showing&&mainEdit.showing.note === notemessage.noteId){
...
function getBackendPage(id,page,pageele,mode) {
    var c_code = get_cookie("c_code");
    if(c_code === null){
        location.href = "login.jsp";
    }
    ...
    $.ajax({
        ...
        success:function(data){
            if((data+"").trim() === "You are not logged in."){
                location.href = "login.jsp";
            }else if((data+"").trim() === "Note doesn't exist." || (data+"").trim() === "Note has not been shared.") {
...
function shareNoteWithBackend() {
    var c_code = get_cookie("c_code");
    if(c_code === null){
        location.href = "login.jsp";
    }
    if(!mainEdit.showing)return;
    $.ajax({
        type: 'POST',
        dataType: 'text',
        url: "ShareNote",
        data:{"c_code":c_code,"type":"shared_mode","note_id":mainEdit.showing.note},
        async:false,
        success:function(data){
            if((data+"").trim() === "You are not logged in."){
                location.href = "login.jsp";
...
function notShareNoteWithBackend(id) {
    var c_code = get_cookie("c_code");
    if(c_code === null){
        location.href = "login.jsp";
    }
    $.ajax({
        ...
        dataType: 'text',
        url: "ShareNote",
        data:{"c_code":c_code,"type":"delete","note_id":id},
        async:false,
        success:function(data){
            if((data+"").trim() === "You are not logged in."){
                location.href = "login.jsp";
            }else if((data+"").trim() === "Note doesn't exist.") {
                alert("取消分享失败，该篇笔记不存在");
            }else {
